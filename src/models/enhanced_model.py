...
class EnhancedSyntaxGCN(torch.nn.Module):
    def __init__(self, num_node_types, hidden_dim=64, num_layers=3, dropout=0.3):
        super().__init__()
        # Input layer (updated for 11 features)
        self.conv1 = GCNConv(11, hidden_dim)
...
class AttentionSyntaxGCN(torch.nn.Module):
    def __init__(self, num_node_types, hidden_dim=64, num_heads=4, dropout=0.3):
        ...
        # Input projection (updated for 11 features)
        self.input_proj = torch.nn.Linear(11, hidden_dim)
        # Attention layers
    ...
    def forward(self, data):
        ...
        # Input projection
        x = self.input_proj(x)
        # Attention layers (not shown for brevity)
        # ...
        # Graph convolution layers
        x = self.conv1(x, edge_index)
        x = self.bn1(x)
...
class UnifiedSyntaxGCN(torch.nn.Module):
    def __init__(self, num_node_types, hidden_dim=64, num_layers=3, dropout=0.3, num_error_types=6):
        super().__init__()
        self.conv1 = GCNConv(11, hidden_dim)
        self.bn1 = BatchNorm1d(hidden_dim)
        self.conv_layers = torch.nn.ModuleList()
        ...
        self.lin2 = torch.nn.Linear(hidden_dim, hidden_dim // 2)
        # Two heads:
        self.validity_head = torch.nn.Linear(hidden_dim // 2, 2)  # valid/invalid
        self.error_type_head = torch.nn.Linear(hidden_dim // 2, num_error_types)  # error type
        self.bn_final1 = BatchNorm1d(hidden_dim)
        self.bn_final2 = BatchNorm1d(hidden_dim // 2)
    ...
    def forward(self, data):
        ...
        x = F.relu(x)
        x = self.dropout(x)
        validity_logits = self.validity_head(x)
        error_type_logits = self.error_type_head(x)
        return validity_logits, error_type_logits 